...
namespace WireChat.Controllers
{
    public class NotificationHubController : Controller
    {
        ...
        [HttpDelete]
        public async Task RemoveBannedGroupMemberNotification(Guid notificationHubId, string groupMemberUserName)
        {
            var groupMember = await _userManager.FindByNameAsync(groupMemberUserName);

            var removeBannedGroupMemberNotificationCommand = 
                new RemoveBannedGroupMemberNotificationCommand(notificationHubId, Guid.Parse(groupMember.Id));

            await _commandDispatcher.DispatchAsync(removeBannedGroupMemberNotificationCommand);
        }

        [HttpDelete]
        public async Task RemoveCreatedGroupNotification(Guid notificationHubId, Guid groupId)
        {
            var removeCreatedGroupNotificationCommand 
                = new RemoveCreatedGroupNotificationCommand(notificationHubId, groupId);

            await _commandDispatcher.DispatchAsync(removeCreatedGroupNotificationCommand);
        }

        [HttpDelete]
        public async Task RemoveDeclinedContactRequestNotification(Guid notificationHubId, string senderUserName)
        {
            var sender = await _userManager.FindByNameAsync(senderUserName);

            var removeDeclinedContactRequestNotificationCommand 
                = new RemoveDeclinedContactRequestNotificationCommand(notificationHubId, Guid.Parse(sender.Id));

            await _commandDispatcher.DispatchAsync(removeDeclinedContactRequestNotificationCommand);
        }

        [HttpDelete]
        public async Task RemoveIssuedContactRequestNotification(Guid notificationHubId)
        {
            var receiverUserId = Guid.Parse(_userManager.GetUserId(User));

            var removeIssuedContactRequestNotificationCommand 
                = new RemoveIssuedContactRequestNotificationCommand(notificationHubId, receiverUserId);

            await _commandDispatcher.DispatchAsync(removeIssuedContactRequestNotificationCommand);
        }

        [HttpDelete]
        public async Task RemoveReceivedContactRequestNotification(Guid notificationHubId, string senderUserName)
        {
            var sender = await _userManager.FindByNameAsync(senderUserName);

            var removeReceivedContactRequestNotificationCommand 
                = new RemoveReceivedContactRequestNotificationCommand(notificationHubId, Guid.Parse(sender.Id));

            await _commandDispatcher.DispatchAsync(removeReceivedContactRequestNotificationCommand);
        }

        [HttpDelete]
        public async Task RemoveRemovedChatMessageNotification(Guid notificationHubId, Guid chatMessageId)
        {
            var removeRemovedChatMessageNotificationCommand 
                = new RemoveRemovedChatMessageNotificationCommand(notificationHubId, chatMessageId);
